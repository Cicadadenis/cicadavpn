	...
	if [[ -e /etc/debian_version ]]; then
		...
		if [[ $ID == "debian" || $ID == "raspbian" ]]; then
			if [[ $VERSION_ID -lt 9 ]]; then
				echo "⚠️Ваша версия Debian не поддерживается."
				echo ""
			...
			if [[ $MAJOR_UBUNTU_VERSION -lt 16 ]]; then
				...
				echo ""
				echo "Однако, если вы используете Ubuntu >= 16.04 или бета, тогда вы можете продолжить на свой страх и риск."
				echo ""
				until [[ $CONTINUE =~ (y|n) ]]; do
					read -rp "Продолжить? [y/n]: " -e CONTINUE
				done
		...
		echo "Похоже, вы не запускаете этот установщик в системе Debian, Ubuntu, Fedora, CentOS, Amazon Linux 2 или Arch Linux."
		exit 1
	fi
}
...
function installQuestions() {
	echo "Добро пожаловать в установщик CicadaVPN!"
	echo "Cicada_3301"
	echo ""

	echo "Мне нужно задать вам несколько вопросов перед началом настройки."
	echo "Вы можете оставить параметры по умолчанию и просто нажать ввод, если они вас устраивают.."
	echo ""
	echo "Мне нужно знать IPv4-адрес сетевого интерфейса, который вы хотите прослушивать CicadaVPN.."
	...
	if echo "$IP" | grep -qE '^(10\.|172\.1[6789]\.|172\.2[0-9]\.|172\.3[01]\.|192\.168)'; then
		echo ""
		echo "Похоже, этот сервер находится за NAT. Какой у него публичный IPv4-адрес или имя хоста?"
	...
	if eval "$PING6"; then
		echo "У вашего хоста есть возможность подключения по IPv6.."
		SUGGESTION="y"
	else
		echo "У вашего хоста нет возможности подключения по IPv6."
		SUGGESTION="n"
	fi
	echo ""
	...
	echo ""
	echo "Какой порт вы хотите, чтобы CicadaVPN слушал?"
	...
	echo ""
	echo "Какой протокол вы хотите использовать CicadaVPN?"
	echo "UDP быстрее. Если он недоступен, вам не следует использовать TCP."
	echo "   1) UDP"
	echo "   2) TCP"
	...
	echo ""
	echo "Какие DNS-преобразователи вы хотите использовать с VPN?"
	echo "   1) Текущие системные преобразователи (from /etc/resolv.conf)"
	echo "   2) Самостоятельный DNS-преобразователь (Unbound)"
	...
	until [[ $DNS =~ ^[0-9]+$ ]] && [ "$DNS" -ge 1 ] && [ "$DNS" -le 13 ]; do
		...
		if [[ $DNS == 2 ]] && [[ -e /etc/unbound/unbound.conf ]]; then
			...
			echo "Вы можете разрешить скрипту настроить его, чтобы использовать его с ваших клиентов CicadaVPN."
			echo "Мы просто добавим второй сервер в /etc/unbound/unbound.conf для подсети CicadaVPN.."
			...
			until [[ $DNS2 =~ ^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$ ]]; do
				read -rp "Вторичный DNS (optional): " -e DNS2
				if [[ $DNS2 == "" ]]; then
					break
				fi
			done
	...
	echo ""
	echo "Вы хотите использовать сжатие? Это не рекомендуется, так как атака ВОРАКЛ использует его.."
	until [[ $COMPRESSION_ENABLED =~ (y|n) ]]; do
		read -rp"Активировать сжатие? [y/n]: " -e -i n COMPRESSION_ENABLED
	done
	...
	echo "Если вы не знаете, что делаете, вам следует придерживаться параметров по умолчанию, предоставленных скриптом.."
	echo "Обратите внимание: что бы вы ни выбрали, все варианты, представленные в сценарии, безопасны. (В отличие от значений по умолчанию CicadaVPN)"
	echo "CicadaVPN."
	echo ""
	...
	if [[ $CUSTOMIZE_ENC == "n" ]]; then
		...
		echo "   1) AES-128-GCM (рекомендуемые)"
		echo "   2) AES-192-GCM"
		echo "   3) AES-256-GCM"
		echo "   4) AES-128-CBC"
		echo "   5) AES-192-CBC"
		...
		1)
			echo ""
			echo "Выберите кривую, которую вы хотите использовать для ключа сертификата:"
			echo "   1) prime256v1 (рекомендуемые)"
			echo "   2) secp384r1"
			echo "   3) secp521r1"
		...
		2)
			...
			echo "   1) 2048 bits (рекомендуемые)"
			echo "   2) 3072 bits"
			echo "   3) 4096 bits"
		...
		case $CERT_TYPE in
		1)
			echo "   1) ECDHE-ECDSA-AES-128-GCM-SHA256 (рекомендуемые)"
			...
			;;
		2)
			echo "   1) ECDHE-RSA-AES-128-GCM-SHA256 (рекомендуемые)"
		...
		echo "   1) tls-crypt (рекомендуемые)"
		echo "   2) tls-auth"
	...
	echo ""
	echo "Ладно, это все, что мне нужно. Мы готовы настроить ваш OpenVPN сервер прямо сейчас."
	echo "Вы сможете сгенерировать клиента в конце установки."
	...
	if [[ ! -e /etc/openvpn/server.conf ]]; then
		if [[ $OS =~ (debian|ubuntu) ]]; then
			...
			if [[ $VERSION_ID == "16.04" ]]; then
				echo "deb CicadaVPN" >/etc/apt/sources.list.d/openvpn.list
...
function newClient() {
	...
	echo "(например зашифровать закрытый ключ паролем)"
	echo "   1) Добавить клиента без пароля"
...
function revokeClient() {
	...
	echo ""
	echo "Сертификат для клиента $CLIENT revoked."
}
...
function removeUnbound() {
	...
	if [[ $REMOVE_UNBOUND == 'y' ]]; then
		...
		echo ""
		echo "Несвязанный удален!"
	else
		systemctl restart unbound
		echo ""
		echo "Несвязанный не был удален."
	fi
}

function removeOpenVPN() {
	echo ""
	read -rp "Вы действительно хотите удалить CicadaVPN? [y/n]: " -e -i n REMOVE
	if [[ $REMOVE == 'y' ]]; then
...
function manageMenu() {
	echo "Добро пожаловать в CicadaVPN"
	echo "CicadaVPN"
	echo ""
	echo "Похоже, CicadaVPN уже установлен."
	echo ""
	echo "What do you want to do?"
	echo "   1) Добавить нового пользователя"
	echo "   2) Удалить существующего пользователя"
	echo "   3) Удалить CicadaVPN"
	echo "   4) Выход"
